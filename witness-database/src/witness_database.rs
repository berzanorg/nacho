use crate::{error::Result, WitnessDatabaseError};
use common::{
    create_poseidon_hasher, field_from_bytes, field_to_bytes, poseidon_hash, Field, PoseidonHasher,
};
...
/// The database which is based on an on-disk Merkle tree optimized for low storage usage and high performance.
///
/// The constant generic `H` represents the height of the Merkle tree.
///
...
///
/// ```rs
/// // Constructs a witness database.
/// let wdb = WitnessDatabase::<42>::new("/tmp/nacho/wdb/").await?;
///
/// // Sets the value at the given index.
/// wdb.set(0, value).await?;
///
/// // Gets the value at the given index.
/// let value = wdb.get(0).await?;
///
/// // Gets the Merkle root of the witness database.
/// let root = wdb.root().await?;
///
/// // Gets the Merkle witness of the given index.
/// let witness = wdb.witness(7).await?;
/// ```
///
pub struct WitnessDatabase<const H: usize> {
    files: [File; H],
    zeroes: [Field; H],
    hasher: PoseidonHasher,
}

impl<const H: usize> WitnessDatabase<H> {
    /// The maximum number of leaves in the database.
    const MAX_NUMBER_OF_LEAVES: u64 = u64::pow(2, H as u32 - 1);

    /// Creates a new `WitnessDatabase` at the given path.
    ///
    ///
    /// Witness Database is an on-disk Merkle tree optimized for low storage usage and high performance.
    ///
    /// The height of the Merkle tree is defined by the constant generic parameter `H`.
    ///
    /// Database files are stored in a folder at the given path and they shouldn't be edited manually.
    ///
    ///
    /// # Examples
    ///
    /// ```rs
    /// let wdb = WitnessDatabase::<42>::new().await?;
    /// ```
    ///
    pub async fn new(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let files = files
            .try_into()
            .map_err(|_| WitnessDatabaseError::Infallible)?;
    ...
    /// Returns the Merkle root of the witness database.
    ///
    /// # Examples
    ///
    /// ```rs
    ...
    ///
    /// Returns zero value for indexes that are not used yet.
    ///
    /// # Examples
    ///
    /// ```rs
    /// let value = wdb.get(7).await?;
    /// ```
    ///
    pub async fn get(&mut self, index: u64) -> Result<Field> {
        if index >= Self::MAX_NUMBER_OF_LEAVES {
            return Err(WitnessDatabaseError::NonExistentIndex {
                given_index: index,
    ...
    ///
    /// Returns an error if the index is invalid or unusable.
    ///
    /// # Examples
    ///
    /// ```rs
    /// wdb.set(7, field).await?;
    /// ```
    ///
    pub async fn set(&mut self, index: u64, value: Field) -> Result<()> {
        ...
        if padding > leaves_file_len {
            return Err(WitnessDatabaseError::UnusableIndex {
                given_index: index,
    ...
    /// Sets the given value at the last usable index and returns the index.
    ///
    /// Returns an error if the witness database is full.
    ///
    /// # Examples
    ///
    /// ```rs
    /// let index = wdb.push(field).await?;
    /// ```
    ///
    ...
    /// Returns the Merkle witness of the given index.
    ///
    /// Returns an error if the index is non-existent.
    ///
    /// # Examples
    ///
    /// ```rs
    ...
    /// ```
    ///
    pub async fn witness(&mut self, index: u64) -> Result<[Field; H]> {
...
#[tokio::test]
async fn creates_database() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let dir = "/tmp/nacho/tests/creates_database";

    let _ = WitnessDatabase::<2>::new(dir).await?;
...
async fn sets_and_gets_values() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let mut wdb = WitnessDatabase::<5>::new(dir).await?;

    assert_eq!(wdb.get(0).await?, Field::from(0));

    wdb.set(0, Field::from(42)).await?;

    assert_eq!(wdb.get(0).await?, Field::from(42));

    wdb.set(0, Field::from(7)).await?;

    assert_eq!(wdb.get(0).await?, Field::from(7));

    wdb.set(1, Field::from(5)).await?;

    assert_eq!(wdb.get(1).await?, Field::from(5));
...
async fn pushes_values() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let mut wdb = WitnessDatabase::<4>::new(dir).await?;

    assert_eq!(wdb.get(0).await?, Field::from(0));

    wdb.push(Field::from(42)).await?;

    assert_eq!(wdb.get(0).await?, Field::from(42));

    wdb.push(Field::from(5)).await?;
...
async fn updates_root() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let mut wdb = WitnessDatabase::<6>::new(dir).await?;
...
async fn doesnt_set_unusable_index() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let mut wdb = WitnessDatabase::<42>::new(dir).await?;
    ...
    match wdb
        ...
        .unwrap_err()
    {
        WitnessDatabaseError::UnusableIndex {
            given_index,
            usable_index,
        } => {
...
async fn doesnt_set_non_existent_index() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    match wdb
        ...
        .unwrap_err()
    {
        WitnessDatabaseError::NonExistentIndex {
            given_index,
...
async fn gets_witness() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let witness = wdb.witness(0).await?;
    ...
    wdb.set(0, "42".parse().unwrap()).await?;

    let witness = wdb.witness(1).await?;
